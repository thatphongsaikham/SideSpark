...
- Build Command (recommended เมื่อ Render ไม่มีช่อง Pre-Deploy): `pnpm install --frozen-lockfile --prod=false && pnpm exec prisma migrate deploy && pnpm build`
- Start Command: `pnpm start`
- Node Version: `20` (recommended) หรือ `22`

Optional (ถ้า Render UI มีช่อง Pre-Deploy Command):
...
- ถ้า Render ไม่มีช่อง Pre-Deploy ให้รวม `pnpm exec prisma migrate deploy` ไว้ใน Build Command
